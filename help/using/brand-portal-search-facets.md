...
1. 「[!UICONTROL プロパティ名]」フィールドで、プロパティピッカーを開き、検索に使用するプロパティを選択します。または、プロパティの名前を入力します。例えば、[!UICONTROL `  jcr :content/metadata/dc:title`] や [!UICONTROL `./jcr:content/metadata/dc:title`] を入力します。
