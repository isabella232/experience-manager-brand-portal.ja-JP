...
適切な権限を持つBrand Portalユーザーは、複数のアセットを含む個々のアセットまたはフォルダー（.zip ファイル）を投稿フォルダーにアップロードできます。 ユーザーは、複数のアセットをアセット投稿フォルダーにアップロードできます。 ただし、一度に作成できるフォルダーは 1 つだけです。

Brand Portalユーザーは、 **新規** サブフォルダー。 **SHARED** フォルダーは、要件とベースラインアセットを配布するためのものです。
...
Brand Portalでは、アスタリスク (&#42;) を検索クエリ内で使用し、検索対象フレーズ内の単語の一部を指定します。

**使用例。検索対象フレーズに出現する正確な単語が不明な場合は、ワイルドカード検索を使用して検索クエリを補完できます。

例： **登る&#42;** 文字で始まる単語を含むすべてのアセットを返します **登る** Brand Portalの検索フォームで、アセットのタイトルの部分検索に「プロパティの述語」が使用されている場合のタイトルフレーズ。

![](assets/wildcard-prop.png)

さらに、以下のような指定ができます。

* **&#42;登る** は、文字で終わる単語を含むすべてのアセットを返します **登る** という名前を付けます。

* **&#42;登る&#42;** 次の文字を含む単語を持つすべてのアセットを返します **登る** という名前を付けます。
